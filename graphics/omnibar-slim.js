    ...
    const app = new Vue({
        ...
        template: `<div class="zd-omnibar">
            ...
            <div class="zd-omnibar__logo">
                <transition name="zd-omnibar__logo-transition">
                    <img v-if="expandLogo" src="../shared/images/ZDM25_BarLogo72.png" />
                </transition>
                <transition name="zd-omnibar__logo-transition">
                    <div style="overflow: visible clip;"><img v-if="!expandLogo" style="left: 0px; height: 100%; /*transform: scale(1.6) translate(10px, 4px);*/" src="../shared/images/ZDM25_BarLogo72.png" /></div>
                </transition>
                <transition name="zd-omnibar__logo-transition">
                    <img v-if="!expandLogo" style="right: 0px;" src="../shared/images/STREAM-Logo_RGB_primary-full-color.png" />
                </transition>
            </div>
        ...
        created: function() {
            ...
            this._nodecgReplicants["message"].on("change", (newVal, oldVal) => {
                ...
                activeSlot.message = newVal.label ? `<string>${newVal.label}: </strong>${newVal.message}` : newVal.message;
                activeSlot.messageClass = `zd-omnibar__ticker__active--full`;
                stagingSlot.messageClass = `zd-omnibar__ticker__staging--full`;

                setTimeout(() => {
                    const messageRef = this.showSlot1 ? this.$refs.slot1Message : this.$refs.slot2Message;
                    console.log(this.$refs.boundary.getBoundingClientRect().width, messageRef.getBoundingClientRect().width);
